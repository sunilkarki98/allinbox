...
export function DashboardSidebar() {
    ...
    return (
        <aside
            className={cn(
                'relative flex-col h-full bg-sidebar text-sidebar-foreground border-r border-sidebar-border transition-all duration-300 hidden md:flex',
                collapsed ? 'w-16' : 'w-64'
            )}
        >
            ...
            <Button
                variant="ghost"
                size="icon"
                className="absolute -right-3 top-6 h-6 w-6 rounded-full border border-border bg-sidebar shadow-md z-10 hover:bg-accent"
            ...
            <div className={cn(
                ...
                <Link href="/dashboard/inbox" className="flex items-center gap-2 overflow-hidden">
                    <div className="h-8 w-8 min-w-8 rounded-lg flex items-center justify-center text-white font-bold shadow-md bg-gradient-to-br from-emerald-500 to-teal-600 shrink-0">
                        U
                    </div>
                    {!collapsed && (
